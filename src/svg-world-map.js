...
var svgWorldMap = (function () {
  ...
  function initMap() {
    ...
    if (document.getElementById("svg-world-map-container") == null) {
      ...
      if (smallScreen != false || options.bigMap == false) {
        ...
        svg.setAttribute(
          "data",
          options.libPath + "world-states-provinces.svg"
        );
      }
  ...
  function pathSetAttributes(path, event) {
    if (path != undefined && path.id != "World" && path.id != "Ocean") {
      ...
      if (
        ...
        typeof event === "string" &&
        (event.substr(0, 1) == "#" || event.substr(0, 3) == "rgb")
      ) {
  ...
  function initCoords() {
    ...
    baseNode.addEventListener("mousemove", function (event) {
      ...
      var svgPoint = basePoint.matrixTransform(
        baseNode.getScreenCTM().inverse()
      );
  ...
  function callBack(event, data) {
    ...
    if (
      ...
      event == "coords" &&
      window[options.mapCoords] &&
      typeof window[options.mapCoords] === "function"
    ) {
  ...
  function initSilder() {
    if (timeData != false) {
      ...
      document.getElementById("map-slider-container").style.display =
        "list-item";
  ...
  function updateControls() {
    if (paused) {
      document.getElementById("map-control-play-pause").innerHTML =
    ...
    if (document.getElementById("map-controls") == null) {
      ...
      for (var element in controlElements) {
        ...
        if (controlElements[element].tag == "button") {
          ...
          window[element].setAttribute(
            "onclick",
            controlElements[element].click
          );
        }
      }
